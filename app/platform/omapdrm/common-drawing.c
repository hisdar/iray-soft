...
static struct yuv rgb_to_yuv_pixel(struct rgb rgb)
{
	...
	struct yuv yuv = {0};
	yuv.y = MAKE_YUV_601_Y(r, g, b);
	yuv.u = MAKE_YUV_601_U(r, g, b);
	yuv.v = MAKE_YUV_601_V(r, g, b);
...
static void drm_draw_test_img(struct framebuffer *fb)
{
	uint32_t *p;
	for (unsigned i = 0; i < 640; i++) {
		for (unsigned j = 0; j < 576; j++) {
			p = (uint32_t*)(fb->planes[0].map + fb->planes[0].stride * j + i * 4);
			*p = MAKE_RGB(255, 255, 255);
		}
	}
}
...
void drm_draw_test_pattern(struct framebuffer *fb, int pattern)
{
	...
	struct framebuffer new_fb = {0};
	new_fb.width = fb->width;
	new_fb.height = fb->height;
	new_fb.format = DRM_FORMAT_XRGB8888;
	new_fb.num_planes = 1;
	//new_fb.planes[0] = {0};

	new_fb.planes[0].stride = fb->width * 4;
	new_fb.planes[0].size = fb->width * 4 * fb->height;

	fb = &new_fb;
	size_t size = fb->planes[0].stride * fb->height;
	fb->planes[0].map = (uint8_t *)malloc(size);
