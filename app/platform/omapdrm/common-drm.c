...
{
	for (unsigned i = 0; i < ARRAY_SIZE(format_info_array); ++i) {
	...
	struct framebuffer *buf)
{
	...
	for (int i = 0; i < format_info->num_planes; ++i) {
		...
		creq.width = buf->width / pi->xsub;
		creq.height = buf->height / pi->ysub;
		creq.bpp = pi->bitspp;
		...
		*/
		...
		buf->planes[i].map = (uint8_t *)mmap(0, plane->size, PROT_READ | PROT_WRITE, MAP_SHARED,
			        fd, mreq.offset);
...
{
	for (unsigned i = 0; i < ARRAY_SIZE(reserved_plane_ids); ++i) {
...
uint32_t drm_reserve_plane(int fd)
{
	...
	for (unsigned i = 0; i < res->count_planes; i++) {
