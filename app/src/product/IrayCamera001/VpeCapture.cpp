...
int VpeCapture::init(int width, int height, int pixel_format)
{
	int ret = 0;
	// 1 init and set fmt
	ret = initAndSetFmt(width, height, pixel_format);
	if (ret) {
		iray_err("initAndSetFmt fail, ret=%d\n", ret);
		return ret;
	}

	// 2. request buffers and save buffer info
	...
	// 3. query buffers and mmap buffers
...
int VpeCapture::queue(int index)
{
	int num_planes = 0;
...
int VpeCapture::queueAllBuffers()
{
	int ret = 0;
...
void VpeCapture::printV4l2Buffer(struct v4l2_buffer &buf)
{
	iray_info("=====================\n");
	iray_info("buf.type     =%d\n", buf.type);
	iray_info("buf.index    =%d\n", buf.index);
	iray_info("buf.field    =%d\n", buf.field);
	iray_info("buf.length   =%d\n", buf.length);
	iray_info("buf.memory   =%d\n", buf.memory);
	iray_info("buf.flags    =%d\n", buf.flags);
	iray_info("buf.bytesused=%d\n", buf.bytesused);
}

int VpeCapture::requestAndSaveBuffer()
{
	int ret = 0;
...
int VpeCapture::initAndSetFmt(int width, int height, int pix_fmt)
{
	int ret = 0;
...
int VpeCapture::initBuffer(struct v4l2_buffer &buf)
{
	void *planes = NULL;
...
void VpeCapture::freeBuffer(struct v4l2_buffer &buf)
{
...
int VpeCapture::initBufferManager(struct v4l2_requestbuffers &req_buf)
{
	u32 i = 0;
	int ret = 0;
	int mem_size = 0;
...
void VpeCapture::freeBufferManager()
{
	// step 1: free buffers
	for (u32 i = 0; i < m_buf_mag.size; i++) {
...
int VpeCapture::initBufferUser(struct v4l2_buffer_user &buf_usr, int num_planes)
{
	void *planes = NULL;
...
void VpeCapture::freeBufferUser(struct v4l2_buffer_user &buf_usr)
{
...
int VpeCapture::initBufferManagerUser(struct v4l2_requestbuffers &req_buf)
{
	u32 i = 0;
	int ret = 0;
	int mem_size = 0;
...
void VpeCapture::freeBufferManagerUser()
{
...
int VpeCapture::req_buf()
{
	...
	if (m_req_buf.count != NUM_OF_DST_BUFFERS) {
		iray_warning("NUM_OF_DST_BUFFERS=%d, m_req_buf.count=%d\n", NUM_OF_DST_BUFFERS, m_req_buf.count);
	}

	return SUCCESS;
}

int VpeCapture::queryAndMmapBufs()
{
	int ret = 0;
...
void VpeCapture::munmap_buffers()
{
	for (u32 i = 0; i < m_buf_mag_usr.size; i++) {
...
int VpeCapture::mmap_buffer(struct v4l2_buffer &buf, struct v4l2_buffer_user &buf_user)
{
	u32 i = 0;
	void *mmap_addr = NULL;
...
void VpeCapture::munmap_buffer(struct v4l2_buffer_user &buf_user)
{
...
int VpeCapture::capture(void *data, u32 len)
{
	int ret = 0;

	ret = m_vpeDev->dq_buf(m_buf_for_dq);
	if (ret) {
		iray_err("dq_buf fail, ret=%d\n", ret);
		return ret;
	}
	...
	ret = queue(m_buf_for_dq.index);
	if (ret) {
		iray_err("queue fail, ret=%d\n", ret);
	}
