...
int FrameBufferDev::outputImage(IrayRgbImage *img, int x, int y, int pix)
{
    int ret = 0;
    IrayRgbImage fbImage;

    ret = fbImage.createFromExternalMem(m_fb_mem_addr, 1920, 1080, COLOR_TYPE_ARGB);
    if (ret ) {
        iray_err("createFromExternalMem fail\n");
        return ret;
    }

    ret = fbImage.draw(img, x, y, 128);
...
int FrameBufferDev::receiveFrame(IrayCameraData *frameData)
{
    ...
    int width = frameData->getImageWidth();
    int height = frameData->getImageHeight();
    
    //iray_dbg("image size[%d, %d], addr[0x%X]\n", width, height, (u32)src);
    ret = img.create(width, height, COLOR_TYPE_ARGB);
    if (ret) {
        iray_err("image create fail, ret=%d\n", ret);
        return ret;
    } else {
        //iray_dbg("create image success\n");
    }

    ret = img.formatFromYUY2(src, width, height);
    if (ret) {
        iray_err("img.formatFromYUY2 fail, ret = %d\n", ret);
        return ret;
    } else {
        //iray_dbg("img.formatFromYUY2 success\n");
    }
    
    return outputImage(&img, 640, 252, 128);
}